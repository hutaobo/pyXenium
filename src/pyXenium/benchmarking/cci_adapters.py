...
EXTERNAL_ATTEMPT_METHODS = {"giotto", "spatalk", "niches"}
...
def validate_input_manifest(manifest: Mapping[str, Any], *, require_full: bool = False) -> dict[str, Any]:
    ...
    if not manifest.get("cci_resource_common_tsv"):
        issues.append("missing cci_resource_common_tsv")
    elif not Path(str(manifest["cci_resource_common_tsv"])).exists():
        issues.append(f"cci_resource_common_tsv does not exist: {manifest['cci_resource_common_tsv']}")
...
def _benchmark_root_for_spec(spec: MethodRunSpec) -> Path:
    if spec.benchmark_root is not None:
        return Path(spec.benchmark_root)
    return resolve_layout(relative_root=ATERA_BENCHMARK_RELATIVE_ROOT).root


def _external_source_dir(spec: MethodRunSpec, method: str) -> Path:
    method_key = method.lower()
    env_key = f"{method_key.upper()}_SRC"
    candidates: list[Path] = []
    if os.environ.get(env_key):
        candidates.append(Path(str(os.environ[env_key])))
    if os.environ.get("PDC_CCI_ROOT"):
        candidates.append(Path(str(os.environ["PDC_CCI_ROOT"])) / "external_src" / method_key)
    candidates.append(_benchmark_root_for_spec(spec) / "external_src" / method_key)
    for candidate in candidates:
        if candidate.exists():
            return candidate
    checked = ", ".join(str(path) for path in candidates)
    raise FileNotFoundError(f"Source checkout for {method_key} was not found. Checked: {checked}")
...
def _filter_cci_resource_to_present_genes(cci_resource: pd.DataFrame, adata: ad.AnnData) -> pd.DataFrame:
    genes = {str(gene) for gene in adata.var_names}
    selected = cci_resource.loc[
        cci_resource["ligand"].astype(str).isin(genes) & cci_resource["receptor"].astype(str).isin(genes)
    ].drop_duplicates(["ligand", "receptor"])
    if selected.empty:
        raise ValueError("No common CCI pairs have both ligand and receptor genes present in the selected AnnData.")
    return selected.reset_index(drop=True)


def _celltype_pair_from_token(token: str, labels: Sequence[str]) -> tuple[str, str]:
    exact = {(f"{sender}_{receiver}", sender, receiver) for sender in labels for receiver in labels}
    for candidate, sender, receiver in exact:
        if token == candidate:
            return sender, receiver
    if "_" in token:
        sender, receiver = token.rsplit("_", 1)
        return sender, receiver
    return token, token


def read_cci_resource(
    ...
    *,
    ...
    if mode == "smoke-panel":
        ...
        path_key = "cci_resource_common_tsv"
    ...
    if missing:
        raise ValueError(f"CCI resource {manifest[path_key]!r} is missing columns: {missing}")
    ...
    if max_cci_pairs is not None and max_cci_pairs > 0:
        resource = resource.head(int(max_cci_pairs)).copy()
    return resource


def _select_cci_chunk(
    ...
    *,
    ...
    if num_chunks is not None or chunk_id is not None:
        if num_chunks is None or chunk_id is None:
            raise ValueError("Both chunk_id and num_chunks must be provided for CCI chunking.")
        if num_chunks <= 0:
    ...
    if max_cci_pairs is not None and max_cci_pairs > 0:
        selected = selected.head(int(max_cci_pairs)).copy()
    return selected
...
def aggregate_cci_by_spatial_neighbors(
    adata: ad.AnnData,
    cci_resource: pd.DataFrame,
    *,
    connectivity_key: str = "spatial_connectivities",
    global_scores: pd.DataFrame | None = None,
    global_score_col: str = "CCI_score",
    ...
    for _, interaction in cci_resource.iterrows():
        ...
        for sender_idx, sender in enumerate(labels):
            for receiver_idx, receiver in enumerate(labels):
                ...
                row = {
                    ...
                    "CCI_score": local_score * positive_weight,
                    "local_cci_score": local_score,
                    "edge_count": edges,
...
def aggregate_cci_by_celltype_means(adata: ad.AnnData, cci_resource: pd.DataFrame) -> pd.DataFrame:
    labels, _, one_hot = _celltype_one_hot(adata.obs["cell_type"].astype(str).to_numpy())
...
def _prepare_expression_adata(adata: ad.AnnData, *, normalize: bool = True) -> ad.AnnData:
    ...
    if "cell_type" not in prepared.obs.columns:
        raise ValueError("AnnData.obs must contain 'cell_type' for CCI benchmark adapters.")
...
def _split_interaction_token(value: Any) -> tuple[str, str]:
...
def flatten_squidpy_cci_result(result: Mapping[str, Any]) -> pd.DataFrame:
    ...
    for row_pos, idx in enumerate(means.index):
        if row_pos < len(metadata_df):
            ...
            if pd.isna(ligand) or pd.isna(receptor):
                ligand, receptor = _split_interaction_token(idx)
        else:
            ligand, receptor = _split_interaction_token(idx)
...
def _liana_interaction_pairs(local_scores: ad.AnnData) -> list[tuple[str, str]]:
    ...
    for idx, name in enumerate(local_scores.var_names.astype(str)):
        ...
        if pd.isna(ligand) or pd.isna(receptor):
            ligand, receptor = _split_interaction_token(name)
...
def aggregate_liana_bivariate_result(
    ...
    *,
    ...
    interaction_pairs = _liana_interaction_pairs(local_scores)
    ...
    for start in range(0, local_scores.n_vars, batch_size):
        ...
        for receiver_idx, receiver in enumerate(labels):
            ...
            for local_col, (ligand, receptor) in enumerate(interaction_pairs[start:end]):
                for sender_idx, sender in enumerate(labels):
...
def run_squidpy_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    try:
        ...
        if "FSStore" in str(exc) or "zarr.storage" in str(exc):
            raise ImportError("Squidpy import failed because the active environment has an incompatible zarr/ome-zarr stack. Rebuild pyx-cci-squidpy with the benchmark env file, which pins zarr<3.") from exc
        raise

    manifest = load_input_manifest(spec.input_manifest)
    cci_resource = read_cci_resource(manifest, database_mode=spec.database_mode, max_cci_pairs=spec.max_cci_pairs)
    ...
    interactions = None
    if cci_resource is not None:
        interactions = cci_resource.rename(columns={"ligand": "source", "receptor": "target"}).loc[:, ["source", "target"]]
    ...
    raw = flatten_squidpy_cci_result(result)
...
def _call_liana_bivariate(adata: ad.AnnData, cci_resource: pd.DataFrame | None, *, n_perms: int | None = None) -> ad.AnnData:
    ...
    if cci_resource is not None:
        kwargs["resource"] = cci_resource.loc[:, ["ligand", "receptor"]].copy()
...
def run_liana_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    local_scores = _call_liana_bivariate(adata, cci_resource, n_perms=spec.n_perms if spec.n_perms > 0 else None)
    raw_dir = spec.output_dir / "raw"
    raw_dir.mkdir(parents=True, exist_ok=True)
...
def _commot_native_resource() -> pd.DataFrame:
    ...
    for kwargs in (
        ...
        try:
            ...
            continue
    raise RuntimeError("Could not load a COMMOT native cell-cell interaction database.")
...
def run_commot_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    manifest = load_input_manifest(spec.input_manifest)
    cci_resource = read_cci_resource(manifest, database_mode=spec.database_mode, max_cci_pairs=None)
    if cci_resource is None:
        cci_resource = _commot_native_resource()
    cci_resource = _select_cci_chunk(
        cci_resource,
        chunk_id=spec.chunk_id,
        num_chunks=spec.num_chunks,
        max_cci_pairs=spec.max_cci_pairs,
    )
    cci_resource = cci_resource.loc[:, ["ligand", "receptor", "pathway"]].copy()
    ...
    raw = aggregate_commot_obsp_result(adata, cci_resource, database_name=database_name)
...
def run_spatialdm_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    available = cci_resource[cci_resource["ligand"].isin(adata.var_names) & cci_resource["receptor"].isin(adata.var_names)].reset_index(drop=True)
    ...
    global_scores = pd.concat(global_rows, ignore_index=True) if global_rows else pd.DataFrame(columns=["ligand", "receptor", "CCI_score"])

    raw_dir = spec.output_dir / "raw"
    raw_dir.mkdir(parents=True, exist_ok=True)
    global_path = _write_tsv(raw_dir / "spatialdm_global.tsv", global_scores)
    raw = aggregate_cci_by_spatial_neighbors(adata, available, global_scores=global_scores, pvalue_col="pvalue")
    ...
    standardized_path, standardized = _write_standardized(
        ...
        pvalue_col="pvalue",
        extra_numeric_cols=("local_cci_score", "edge_count", "spatial_coherence", "spatialdm_moran_r", "spatialdm_zscore"),
        gzip_output=spec.gzip_standardized,
    )
    return {
        "method": "spatialdm",
...
def run_stlearn_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    if cci_resource is None:
        ...
        cci_resource = pd.DataFrame([_split_interaction_token(value) for value in native_lrs], columns=["ligand", "receptor"])
        cci_resource["pathway"] = "stlearn_native"
    cci_resource = _select_cci_chunk(
        cci_resource,
        chunk_id=spec.chunk_id,
        num_chunks=spec.num_chunks,
    ...
    lrs = np.asarray([f"{row.ligand}_{row.receptor}" for row in cci_resource.itertuples(index=False)], dtype=str)
    ...
    if summary.empty:
        global_scores = pd.DataFrame(columns=["ligand", "receptor", "CCI_score"])
    else:
        summary = summary.reset_index(names="interaction_pair")
        pairs = summary["interaction_pair"].map(_split_interaction_token)
        ...
        summary["CCI_score"] = pd.to_numeric(summary.get(score_col, 0), errors="coerce").fillna(0.0) + 1e-12
        global_scores = summary.loc[:, [col for col in ["ligand", "receptor", "CCI_score", "n_spots", "n_spots_sig", "n_spots_sig_pval"] if col in summary.columns]]
    ensure_spatial_connectivities(adata, key="spatial_connectivities")

    raw_dir = spec.output_dir / "raw"
    raw_dir.mkdir(parents=True, exist_ok=True)
    summary_path = _write_tsv(raw_dir / "stlearn_cci_summary.tsv", global_scores)
    raw = aggregate_cci_by_spatial_neighbors(adata, cci_resource, global_scores=global_scores, pvalue_col=None)
    ...
    standardized_path, standardized = _write_standardized(
        ...
        database_mode=spec.database_mode,
        resolution="local_cci",
        spatial_support_type="significant_cci_hotspot_neighbor",
        extra_numeric_cols=("local_cci_score", "edge_count", "spatial_coherence", "n_spots", "n_spots_sig", "n_spots_sig_pval"),
        gzip_output=spec.gzip_standardized,
    )
    return {
        "method": "stlearn",
...
def run_cellphonedb_baseline_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    raw_adata, _ = load_adata_from_manifest(manifest, spec.phase)
    adata = _prepare_expression_adata(raw_adata, normalize=True)
    raw = aggregate_cci_by_celltype_means(adata, cci_resource)
    raw_dir = spec.output_dir / "raw"
...
def run_laris_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    standardized_path, standardized = _write_standardized(
        ...
        database_mode=spec.database_mode,
        resolution="celltype_pair",
        spatial_support_type="knn_diffusion_cci",
...
def _cellnest_command(script: Path, args: Sequence[str], *, source_dir: Path, work_dir: Path) -> list[str]:
    image = os.environ.get("CELLNEST_CONTAINER") or os.environ.get("CELLNEST_IMAGE")
    runtime = shutil.which("apptainer") or shutil.which("singularity")
    if image and Path(image).exists() and runtime:
        return [
            runtime,
            "exec",
            "--bind",
...
def _parse_cellnest_allccc(path: Path, source_adata: ad.AnnData) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"CellNEST postprocess output was not found: {path}")
    ...
    missing = [col for col in ("from_cell", "to_cell", "ligand", "receptor") if col not in table.columns]
    if missing:
        raise ValueError(f"CellNEST allCCC output is missing required columns: {missing}")
    table = table.rename(columns=rename).copy()
    obs = source_adata.obs.copy()
    obs.index = obs.index.astype(str)
    celltype = obs["cell_type"].astype(str).to_dict()
    table["sender_celltype"] = table["from_cell"].astype(str).map(celltype)
    table["receiver_celltype"] = table["to_cell"].astype(str).map(celltype)
    table = table.dropna(subset=["sender_celltype", "receiver_celltype"])
    if table.empty:
        raise ValueError("CellNEST allCCC output did not contain cell IDs matching the benchmark AnnData.")
    score_col = "attention_score" if "attention_score" in table.columns else "edge_rank"
    table[score_col] = pd.to_numeric(table[score_col], errors="coerce")
    if score_col == "edge_rank":
        table["CCI_score"] = 1.0 / table["edge_rank"].replace(0, np.nan)
    else:
        table["CCI_score"] = table[score_col]
    table = table.replace([np.inf, -np.inf], np.nan).dropna(subset=["CCI_score"])
    grouped = (
        table.groupby(["ligand", "receptor", "sender_celltype", "receiver_celltype"], as_index=False)
        .agg(CCI_score=("CCI_score", "mean"), cell_pair_count=("CCI_score", "size"))
    )
    return grouped


def run_cellnest_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    manifest = load_input_manifest(spec.input_manifest)
    cci_resource = read_cci_resource(manifest, database_mode=spec.database_mode, max_cci_pairs=spec.max_cci_pairs)
    if cci_resource is None:
        raise NotImplementedError("CellNEST native-db mode is not supported by this standardized adapter; use common-db.")
    raw_adata, _ = load_adata_from_manifest(manifest, spec.phase)
    raw_adata = _prepare_expression_adata(raw_adata, normalize=False)
    cci_resource = _filter_cci_resource_to_present_genes(cci_resource, raw_adata)

    source_dir = _external_source_dir(spec, "cellnest")
    raw_dir = spec.output_dir / "raw"
    work_dir = raw_dir / "cellnest_work"
    input_dir = work_dir / "input"
    input_dir.mkdir(parents=True, exist_ok=True)
    data_name = f"atera_{spec.phase}_{_safe_slug(spec.database_mode)}"
    model_name = "CellNEST_atera"
    h5ad_path = input_dir / f"{data_name}.h5ad"
    raw_adata.write_h5ad(h5ad_path)
    cci_csv = input_dir / "common_cci_cellnest.csv"
    cellnest_cci = cci_resource.rename(columns={"ligand": "Ligand", "receptor": "Receptor"}).loc[:, ["Ligand", "Receptor"]].copy()
    cellnest_cci["Annotation"] = "Benchmark common-db"
    cellnest_cci["Reference"] = "pyXenium CCI benchmark"
    cellnest_cci.to_csv(cci_csv, index=False)

    epochs_default = "1000" if spec.phase == "full" else "500"
    num_epoch = int(os.environ.get("CELLNEST_NUM_EPOCH", epochs_default))
    hidden = int(os.environ.get("CELLNEST_HIDDEN", "64"))
    common_env = {**os.environ, "PYTHONPATH": f"{source_dir}{os.pathsep}{os.environ.get('PYTHONPATH', '')}"}
    preprocess_args = [
        "--data_name",
        data_name,
        "--data_from",
        ...
        "anndata",
        "--data_to",
        str(work_dir / "input_graph") + os.sep,
        "--metadata_to",
        str(work_dir / "metadata") + os.sep,
        "--database_path",
        str(cci_csv),
        "--skip_normalize",
        "1",
        ...
        "knn",
        "--k",
        os.environ.get("CELLNEST_K", "6"),
    ]
    _run_logged_command(
        _cellnest_command(source_dir / "data_preprocess_CellNEST.py", preprocess_args, source_dir=source_dir, work_dir=work_dir),
        cwd=source_dir,
        stdout_path=raw_dir / "cellnest_preprocess.stdout.log",
        stderr_path=raw_dir / "cellnest_preprocess.stderr.log",
        env=common_env,
    )
    run_args = [
        "--data_name",
        ...
        str(num_epoch),
        "--model_path",
        str(work_dir / "model") + os.sep,
        "--embedding_path",
        str(work_dir / "embedding_data") + os.sep,
        "--training_data",
        str(work_dir / "input_graph") + os.sep,
        "--metadata_to",
        str(work_dir / "metadata") + os.sep,
    ...
    _run_logged_command(
        _cellnest_command(source_dir / "run_CellNEST.py", run_args, source_dir=source_dir, work_dir=work_dir),
        cwd=source_dir,
        stdout_path=raw_dir / "cellnest_run.stdout.log",
        stderr_path=raw_dir / "cellnest_run.stderr.log",
        env=common_env,
    )
    post_args = [
        "--data_name",
    ...
    _run_logged_command(
        _cellnest_command(source_dir / "output_postprocess_CellNEST.py", post_args, source_dir=source_dir, work_dir=work_dir),
        cwd=source_dir,
        stdout_path=raw_dir / "cellnest_postprocess.stdout.log",
        stderr_path=raw_dir / "cellnest_postprocess.stderr.log",
        env=common_env,
    )
    raw_path = work_dir / "output" / data_name / f"{model_name}_allCCC.csv"
    raw = _parse_cellnest_allccc(raw_path, raw_adata)
    if raw.empty:
        raise ValueError("CellNEST completed but produced no standardized CCI rows.")
    flat_path = _write_tsv(raw_dir / "cellnest_celltype_flat.tsv", raw)
    standardized_path, standardized = _write_standardized(
        raw,
        output_dir=spec.output_dir,
        method="cellnest",
        database_mode=spec.database_mode,
        resolution="celltype_pair",
        spatial_support_type="cellnest_attention",
        extra_numeric_cols=("cell_pair_count",),
        gzip_output=spec.gzip_standardized,
    )
    return {
        "method": "cellnest",
        "status": "success",
        "raw_tsv": str(flat_path),
        "cellnest_allccc": str(raw_path),
        "standardized_tsv": str(standardized_path),
        "standardized_tsv_gz": str(standardized_path) if str(standardized_path).endswith(".gz") else None,
        "n_rows": int(len(standardized)),
        "top_hit": standardized.head(1).to_dict(orient="records"),
        "cellnest_num_epoch": num_epoch,
        "cellnest_hidden": hidden,
    }


def run_cellagentchat_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    manifest = load_input_manifest(spec.input_manifest)
    cci_resource = read_cci_resource(manifest, database_mode=spec.database_mode, max_cci_pairs=None)
    if cci_resource is None:
        raise NotImplementedError("CellAgentChat native-db mode is not supported by this standardized adapter; use common-db.")
    cci_resource = _select_cci_chunk(
        cci_resource,
        chunk_id=spec.chunk_id,
        num_chunks=spec.num_chunks,
    ...
    source_dir = _external_source_dir(spec, "cellagentchat")
    source_src = source_dir / "src"
    _prepend_sys_path(source_src)

    from abm import CellModel  # type: ignore
    from model_setup import add_rates, feature_selection, load_tf_db, train  # type: ignore

    raw_dir = spec.output_dir / "raw"
    work_dir = raw_dir / "cellagentchat_work"
    work_dir.mkdir(parents=True, exist_ok=True)
    labels, _, _ = _celltype_one_hot(adata.obs["cell_type"].astype(str).to_numpy())
    cci_tsv = work_dir / "common_cci_cellagentchat.tsv"
    cellagent_cci = pd.DataFrame(
        {
            "pair_id": np.arange(len(cci_resource), dtype=int),
            "ligand": cci_resource["ligand"].astype(str).to_numpy(),
            "receptor": cci_resource["receptor"].astype(str).to_numpy(),
        }
    )
    cellagent_cci.to_csv(cci_tsv, sep="\t", index=False)
    coords = np.asarray(adata.obsm["spatial"], dtype=float)
    x = coords[:, 0]
    y = coords[:, 1]
    ...
    adata.obs["x_true"] = x
    adata.obs["y_true"] = y
    adata.obs["x"] = np.clip(np.rint(((x - float(np.min(x))) / x_span) * 100.0), 0, 100).astype(int)
    adata.obs["y"] = np.clip(np.rint(((y - float(np.min(y))) / y_span) * 100.0), 0, 100).astype(int)
    adata.obs["Batch"] = adata.obs.get("Batch", "benchmark").astype(str) if "Batch" in adata.obs else "benchmark"

    lig_uni: dict[str, list[str]] = {}
    rec_uni: dict[str, list[str]] = {}
    for _, row in cci_resource.iterrows():
        ligand = str(row["ligand"])
        receptor = str(row["receptor"])
        lig_uni.setdefault(ligand, []).append(receptor)
        rec_uni.setdefault(receptor, []).append(ligand)
    interaction_pairs = [f"{ligand}_{receptor}" for ligand, receptors in lig_uni.items() for receptor in receptors]
    old_cwd = Path.cwd()
    os.chdir(source_src)
    try:
        tf_uni, rec_tf_uni = load_tf_db("human", adata, rec_uni)
        epochs = int(os.environ.get("CELLAGENTCHAT_EPOCHS", "50" if spec.phase == "full" else "10"))
        model_path = work_dir / "cellagentchat_model.pt"
        mat, target = train(adata, lig_uni, rec_uni, tf_uni, rec_tf_uni, interaction_pairs, path=str(model_path), epochs=epochs)
    finally:
        os.chdir(old_cwd)
    use_feature_selection = os.environ.get("CELLAGENTCHAT_FEATURE_SELECTION", "1").strip().lower() not in {"0", "false", "no"}
    if use_feature_selection:
        import torch

        try:
            model_dl = torch.load(str(model_path), weights_only=False)
        except TypeError:
            model_dl = torch.load(str(model_path))
        conversion_rates = feature_selection(model=model_dl, mat=mat, C=target, rec_uni=rec_uni)
        rates = add_rates(conversion_rates, rec_uni)
    else:
        rates = {receptor: 1.0 for receptor in rec_uni}
    ...
    model = CellModel(
        ...
        rec_uni=rec_uni,
        rates=rates,
        max_steps=int(os.environ.get("CELLAGENTCHAT_MAX_STEPS", "1")),
        dist=True,
        delta=float(os.environ.get("CELLAGENTCHAT_DELTA", "1")),
        tau=float(os.environ.get("CELLAGENTCHAT_TAU", "2")),
        diff=float(os.environ.get("CELLAGENTCHAT_DIFF", "1")),
        rec_block=False,
        net=str(model_path),
    )
    for _ in range(model.max_steps):
        model.step()
    rows: list[dict[str, Any]] = []
    for cpair, cci_scores in model.results2.items():
        sender, receiver = _celltype_pair_from_token(str(cpair), labels)
        for interaction_token, score in dict(cci_scores).items():
            ligand, receptor = (
                str(interaction_token).split("-", 1)
                if "-" in str(interaction_token)
                else _split_interaction_token(interaction_token)
            )
            value = float(score)
    ...
    raw = pd.DataFrame(rows)
    if raw.empty:
        raise ValueError("CellAgentChat completed but produced no CCI interaction rows.")
    flat_path = _write_tsv(raw_dir / "cellagentchat_celltype_flat.tsv", raw)
    standardized_path, standardized = _write_standardized(
        raw,
        output_dir=spec.output_dir,
        ...
        database_mode=spec.database_mode,
        resolution="celltype_pair",
        spatial_support_type="agent_based_cci",
        gzip_output=spec.gzip_standardized,
    )
    return {
        "method": "cellagentchat",
...
def _scild_cci_database(cci_resource: pd.DataFrame) -> pd.DataFrame:
    ligands = sorted(cci_resource["ligand"].astype(str).unique())
    receptors = sorted(cci_resource["receptor"].astype(str).unique())
    database = pd.DataFrame(0, index=ligands, columns=receptors, dtype=int)
    for _, row in cci_resource.iterrows():
        database.loc[str(row["ligand"]), str(row["receptor"])] = 1
    return database


def run_scild_adapter(spec: MethodRunSpec) -> dict[str, Any]:
    manifest = load_input_manifest(spec.input_manifest)
    cci_resource = read_cci_resource(manifest, database_mode=spec.database_mode, max_cci_pairs=spec.max_cci_pairs)
    if cci_resource is None:
    ...
    scild = SCILD(
        adata=adata,
        LRDatabase_D=cci_database,
        neighbor_k=int(os.environ.get("SCILD_NEIGHBOR_K", "5")),
        niter_max=niter_max,
        eps=float(os.environ.get("SCILD_EPS", "1e-4")),
        verbose=True,
        plot_error=False,
    )
    scild.preparing()
    ...
    labels, _, one_hot = _celltype_one_hot(adata.obs["cell_type"].astype(str).to_numpy())
    rows: list[dict[str, Any]] = []
    p_matrix = scild.P.tocsr()
    cell_indices = np.arange(scild.ns)
    for ligand_idx, ligand in enumerate(scild.LRDatabase_D.index.astype(str)):
        row_index = ligand_idx + cell_indices * scild.nl
        for receptor_idx, receptor in enumerate(scild.LRDatabase_D.columns.astype(str)):
            if scild.LRDatabase_D.iloc[ligand_idx, receptor_idx] != 1:
                continue
            col_index = receptor_idx + cell_indices * scild.nr
            pair_matrix = p_matrix[row_index, :][:, col_index].tocsr()
            aggregate = one_hot.T @ pair_matrix @ one_hot
            edge_counts = one_hot.T @ pair_matrix.astype(bool).astype(float) @ one_hot
            aggregate_arr = np.asarray(aggregate.toarray() if sparse.issparse(aggregate) else aggregate, dtype=float)
            edge_arr = np.asarray(edge_counts.toarray() if sparse.issparse(edge_counts) else edge_counts, dtype=float)
            for sender_idx, sender in enumerate(labels):
                for receiver_idx, receiver in enumerate(labels):
    ...
    raw = pd.DataFrame(rows)
    if raw.empty:
        raise ValueError("SCILD completed but produced no CCI interaction rows.")
    raw_dir = spec.output_dir / "raw"
    flat_path = _write_tsv(raw_dir / "scild_celltype_flat.tsv", raw)
    standardized_path, standardized = _write_standardized(
        raw,
        output_dir=spec.output_dir,
        method="scild",
        database_mode=spec.database_mode,
        resolution="celltype_pair",
        spatial_support_type="scild_probability",
        extra_numeric_cols=("communication_mass", "edge_count", "spatial_coherence"),
        gzip_output=spec.gzip_standardized,
    )
    return {
...
def run_external_attempt_adapter(spec: MethodRunSpec, *, package_candidates: Sequence[str]) -> dict[str, Any]:
    ...
    if not found:
        payload = {
            ...
            "package_candidates": list(package_candidates),
            "reproduce": f"conda run --name <method-env> python -m pyXenium benchmark atera-cci run-method --method {spec.method}",
        }
        _write_json(spec.output_dir / "run_summary.json", payload)
        _write_method_card(spec.output_dir, payload)
        return payload
...
def run_cellchat_subprocess(spec: MethodRunSpec) -> dict[str, Any]:
    ...
    if shutil.which(rscript) is None:
        payload = {
            ...
            "status": "failed",
            "reason": f"Rscript executable was not found: {rscript}",
            "reproduce": f"conda run --name r-cci-cellchat Rscript {script} --method cellchat --input-manifest {spec.input_manifest} --output-dir {spec.output_dir}",
        }
        _write_json(spec.output_dir / "run_summary.json", payload)
        _write_method_card(spec.output_dir, payload)
    ...
    if spec.max_cci_pairs is not None:
        command.extend(["--max-cci-pairs", str(spec.max_cci_pairs)])
...
def build_method_run_plan(
    *,
    ...
    cci_resource = read_cci_resource(manifest, database_mode=mode, max_cci_pairs=max_cci_pairs)
    ...
    return {
        ...
        "interaction_pairs": None if cci_resource is None else int(len(cci_resource)),
...
def run_registered_method(
    *,
    ...
    manifest = load_input_manifest(manifest_path)
    input_source = select_input_source(manifest, phase)
    cci_resource = read_cci_resource(manifest, database_mode=plan["database_mode"], max_cci_pairs=max_cci_pairs)
    ...
    params = {
        **plan,
        ...
        "cci_resource_sha256": None if cci_resource is None else _file_sha256(str(manifest["cci_resource_common_tsv"])) if plan["database_mode"] == "common-db" else None,
    }
    ...
    try:
        if method_key == "pyxenium":
            ...
            cci_path = manifest.get("cci_resource_common_tsv") if plan["database_mode"] == "common-db" else manifest.get("atera_smoke_panel_tsv")
            if max_cci_pairs is not None and cci_path:
                limited = read_cci_resource(manifest, database_mode=plan["database_mode"], max_cci_pairs=max_cci_pairs)
                cci_path = resolved_output_dir / "raw" / "limited_cci_resource.tsv"
                _write_tsv(cci_path, limited if limited is not None else pd.DataFrame())
            ...
            payload = run_cellnest_adapter(spec)
        elif method_key == "cellagentchat":
            payload = run_cellagentchat_adapter(spec)
        elif method_key == "scild":
            payload = run_scild_adapter(spec)
        else:
            raise NotImplementedError(f"Method {method_key!r} does not have a real adapter yet.")
        payload = {
            **payload,
