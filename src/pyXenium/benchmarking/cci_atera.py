...
def harmonize_adata_for_benchmark(adata: ad.AnnData, *, cell_type_col: str = "cluster", copy: bool = True) -> ad.AnnData:
    ...
    if "spatial" not in bench.obsm:
        raise ValueError("AnnData must contain .obsm['spatial'] to participate in the CCI benchmark.")
...
def _default_cci_common_resource(adata: ad.AnnData) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        ...
        resource = DEFAULT_CCI_SMOKE_PANEL.loc[:, ["ligand", "receptor"]].drop_duplicates()
    ...
    if resource.empty:
        ...
        resource = DEFAULT_CCI_SMOKE_PANEL.loc[:, ["ligand", "receptor"]].drop_duplicates().copy()
...
def prepare_atera_cci_benchmark(
    *,
    ...
    common_db, common_db_meta = _default_cci_common_resource(adata)
    common_db["evidence_weight"] = 1.0
    common_db_path = layout.data_dir / "cci_resource_common.tsv"
    common_db.to_csv(common_db_path, sep="\t", index=False)

    smoke_panel_path = layout.data_dir / "atera_smoke_panel.tsv"
    smoke_panel = DEFAULT_CCI_SMOKE_PANEL.copy()
...
def run_pyxenium_smoke(
    *,
    ...
    cci_panel_path: str | Path | None = None,
    ...
    interaction_pairs = pd.read_csv(cci_panel_path, sep="\t") if cci_panel_path else DEFAULT_CCI_SMOKE_PANEL.copy()
    if "evidence_weight" not in interaction_pairs.columns:
        interaction_pairs["evidence_weight"] = 1.0
    ...
    result = cci_topology_analysis(
        ...
        top_n_pairs=min(len(interaction_pairs), 50),
        min_cross_edges=50,
    ...
    payload = {
        ...
        "cci_panel_path": str(cci_panel_path) if cci_panel_path else None,
...
def render_atera_cci_benchmark_report(
    *,
