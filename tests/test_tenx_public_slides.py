...
def test_discover_10x_excludes_intermediate_unzipped_child(tmp_path):
    xenium_root = tmp_path / "Xenium"
    parent = _make_inventory_case(
        xenium_root / "Atera" / "WTA_Preview_FFPE_Cervical_Cancer_outs",
        uuid="atera-uuid",
        run_name="WTA Preview",
    )
    child = parent / "unzipped"
    child.mkdir()
    (child / "experiment.xenium").write_text(
        json.dumps({"experiment_uuid": "atera-uuid", "run_name": "WTA Preview"}),
        encoding="utf-8",
    )

    records = discover_10x_xenium_datasets(xenium_root, include_duplicates=True)

    excluded = [record for record in records if record["case_slug"] == "unzipped"]
    assert len(excluded) == 1
    assert excluded[0]["is_excluded_non_dataset"] is True
    assert excluded[0]["selected_for_build"] is False
    selected_only = discover_10x_xenium_datasets(xenium_root)
    assert [record["case_slug"] for record in selected_only] == ["wta_preview_ffpe_cervical_cancer"]


def test_discover_10x_accepts_10x_he_imagealignment_name(tmp_path):
    xenium_root = tmp_path / "Xenium"
    ...
    (case / "case_he_image.ome.tif").write_bytes(b"placeholder")
...
def test_resolve_10x_dataset_metadata_uses_cache_alias_without_network(tmp_path):
    record = {
        "case_slug": "case_a",
        "case_name": "case_a",
        "xenium_root": str(tmp_path / "case_a"),
    }
    cache = {
        "records": {
            "https://www.10xgenomics.com/datasets/example": {
                "metadata_status": "resolved",
                "source_url": "https://www.10xgenomics.com/datasets/example",
                "title": "Example Xenium Dataset",
                "fields": {"Species": "Human", "Anatomical Entity": "Lung"},
            }
        },
        "aliases": {"case_a": "https://www.10xgenomics.com/datasets/example"},
    }

    resolved = resolve_10x_dataset_metadata(record, metadata_cache=cache, allow_network=False)

    assert resolved["metadata_status"] == "resolved"
    assert resolved["title"] == "Example Xenium Dataset"
...
def test_build_10x_public_slides_reports_excluded_intermediate_not_failed(tmp_path):
    xenium_root = tmp_path / "Xenium"
    parent = _make_inventory_case(
        xenium_root / "Atera" / "WTA_Preview_FFPE_Cervical_Cancer_outs",
        uuid="atera-uuid",
    ...
    child = parent / "unzipped"
    child.mkdir()
    (child / "experiment.xenium").write_text("{}", encoding="utf-8")
    output_case = tmp_path / "out" / "Atera" / "wta_preview_ffpe_cervical_cancer"
    output_case.mkdir(parents=True)
    (output_case / "slide_manifest.json").write_text(
        json.dumps({"counts": {"cells": 1, "genes": 1, "assigned_cells": 0, "contour_patches": 0}}),
        encoding="utf-8",
    )
    ...
    assert summary["not_buildable"] == 0
    assert summary["excluded_non_dataset"] == 1
    failed_rows = json.loads((tmp_path / "out" / "failed_cases.json").read_text(encoding="utf-8"))
    assert failed_rows == []
...
def test_build_10x_public_slides_repairs_existing_contract_artifacts(tmp_path):
    ...
    assert registry[0]["contour_missing"] is True
    assert registry[0]["contour_images_available"] is False


def test_build_10x_public_slides_reads_existing_contour_source_and_readiness(tmp_path):
    xenium_root = tmp_path / "Xenium"
    _make_inventory_case(
        xenium_root / "Xenium_Test" / "case_outs",
        uuid="uuid-existing-contour",
        run_name="Xenium_Prime_Human_Breast_Cancer_FFPE_outs",
    )
    output_case = tmp_path / "out" / "Xenium_Test" / "case"
    output_case.mkdir(parents=True)
    (output_case / "slide_manifest.json").write_text(
        json.dumps(
            {
                "counts": {"cells": 5, "genes": 2, "assigned_cells": 4, "contour_patches": 0},
                "artifacts": {},
            }
        ),
        encoding="utf-8",
    ...
    (output_case / "qc_report.json").write_text(json.dumps({"status": "pass"}), encoding="utf-8")
    (output_case / "contour_source_manifest.json").write_text(
        json.dumps({"status": "generated", "selected_geojson": str(output_case / "generated.geojson")}),
        encoding="utf-8",
    )
    ...
    assert summary["completed_outputs"] == 1
    registry = json.loads((tmp_path / "out" / "dataset_registry.json").read_text(encoding="utf-8"))
    assert registry[0]["contour_status"] == "generated"
    assert registry[0]["contour_assigned"] is True
    assert registry[0]["contour_readiness"] == "contour_assigned_no_image_context"
