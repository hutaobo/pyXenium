...
split_interaction_name <- function(x) {
  dash <- intToUtf8(8212)
...
aggregate_niches_matrix <- function(mat, metadata, artifact_path, database_mode) {
  ...
  interaction_parts <- t(vapply(rownames(mat)[merged$interaction], split_interaction_name, character(2)))
  data.frame(
    method = method,
    database_mode = database_mode,
    ligand = interaction_parts[, 1],
    receptor = interaction_parts[, 2],
