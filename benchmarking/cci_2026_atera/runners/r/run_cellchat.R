...
if (database_mode %in% c("common", "common-db", "smoke-panel")) {
  db_path <- if (database_mode == "smoke-panel") manifest$atera_smoke_panel_tsv else manifest$cci_resource_common_tsv
  lr_db <- read.delim(db_path, sep = "\t", stringsAsFactors = FALSE)
  if (!"pathway" %in% colnames(lr_db)) lr_db$pathway <- "custom"
  if (!is.null(max_cci_pairs)) lr_db <- head(lr_db, as.integer(max_cci_pairs))
