...
def write_commot_gating(layout) -> dict[str, Any]:
    ...
    smoke_pairs = int(smoke_params.get("max_cci_pairs") or 100)
    ...
    payload = {
        ...
        "smoke_cci_pairs": smoke_pairs,
        "full_cells": full_cells,
        "full_cci_pairs": full_pairs,
    ...
    lines = [
        ...
        "",
        "- Status: `appendix_gated`",
        f"- Smoke benchmark: `{smoke_cells}` cells x `{smoke_pairs}` CCI pairs in `{smoke_elapsed:.1f}` seconds.",
        f"- Full target: `{full_cells}` cells x `{full_pairs}` CCI pairs.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Finalize collected A100 full common-db CCI benchmark outputs.")
