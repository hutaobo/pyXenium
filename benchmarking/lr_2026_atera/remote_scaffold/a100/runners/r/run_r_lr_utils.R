...
parse_lr_args <- function(args = commandArgs(trailingOnly = TRUE)) {
  parsed <- list()
  i <- 1
  while (i <= length(args)) {
    key <- args[[i]]
...
read_lr_database <- function(manifest, mode, max_lr_pairs = NULL, chunk_id = NULL, chunk_size = NULL) {
  db_path <- if (mode %in% c("smoke-panel", "smoke_panel")) manifest$atera_smoke_panel_tsv else manifest$lr_db_common_tsv
  if (is.null(db_path) || !file.exists(db_path)) {
    stop("Manifest LR database path is missing or unreadable.", call. = FALSE)
  }
  lr <- read.delim(db_path, sep = "\t", stringsAsFactors = FALSE)
  if (!all(c("ligand", "receptor") %in% colnames(lr))) {
    stop("LR database must contain ligand and receptor columns.", call. = FALSE)
  }
  ...
  if (!is.null(max_lr_pairs)) {
    lr <- head(lr, as.integer(max_lr_pairs))
  }
  lr
}
