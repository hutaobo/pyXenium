...
activate_method_renv <- function(method, input_manifest = NULL, output_dir = NULL) {
  benchmark_root <- Sys.getenv("PYX_LR_BENCHMARK_ROOT", unset = NA_character_)
  ...
  project_dir <- file.path(benchmark_root, "envs", paste0("r-lr-", method, "_project"))
...
parse_args <- function() {
  ...
  while (i <= length(args)) {
    ...
    if (key == "--max-lr-pairs") out$max_lr_pairs <- as.integer(value)
...
run_package_present_expression_baseline <- function(opts, available) {
  ...
  lr_path <- manifest$lr_db_common_tsv
  if (is.null(lr_path) || !file.exists(lr_path)) {
    stop("Manifest lr_db_common_tsv is missing or not readable")
  }
  ...
  if (!all(c("ligand", "receptor") %in% colnames(lr))) {
    stop("LR database must contain ligand and receptor columns")
  }
  ...
  if (!is.null(opts$max_lr_pairs) && !is.na(opts$max_lr_pairs)) {
    lr <- head(lr, opts$max_lr_pairs)
  }
  if (nrow(lr) == 0) {
    stop("No detectable LR pairs remained after filtering")
  }
  ...
  for (i in seq_len(nrow(lr))) {
    ...
    chunks[[i]] <- data.frame(
      ...
      artifact_path = file.path(opts$output_dir, "raw", paste0(opts$method, "_expression_lr_scores.tsv")),
  ...
  raw_path <- file.path(opts$output_dir, "raw", paste0(opts$method, "_expression_lr_scores.tsv"))
  ...
  payload <- list(
    ...
    note = "A method package was importable, but the method-specific public API mapping is not finalized; this bounded result uses the shared LR expression baseline and is kept for appendix comparison."
  )
...
main <- function() {
  ...
  payload <- list(
    ...
    input_manifest = opts$input_manifest,
    output_dir = opts$output_dir,
    reproduce = paste("conda run --name <method-env> Rscript", "run_external_lr_method.R", "--method", opts$method)
  )
  write_json(payload, file.path(opts$output_dir, "run_summary.json"), auto_unbox = TRUE, pretty = TRUE)
