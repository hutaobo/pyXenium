...
ROOT="${PDC_LR_ROOT:-/cfs/klemming/scratch/h/hutaobo/pyxenium_lr_benchmark_2026-04}"
SOURCE_ROOT="${PDC_LR_SOURCE_ROOT:-$ROOT/data/source_cache/breast/WTA_Preview_FFPE_Breast_Cancer_outs}"
PY_ENV="${PDC_LR_PREP_ENV:-$ROOT/envs/python/prep}"
...
python "$ROOT/repo/benchmarking/lr_2026_atera/scripts/prepare_data.py" \
...
if [ -f "$ROOT/data/staged_common/lr_db_common.tsv" ]; then
  cp -f "$ROOT/data/staged_common/lr_db_common.tsv" "$ROOT/data/lr_db_common.tsv"
...
root = Path(os.environ.get("PDC_LR_ROOT", "/cfs/klemming/scratch/h/hutaobo/pyxenium_lr_benchmark_2026-04"))
manifest_path = root / "data" / "input_manifest.json"
manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
lr_path = root / "data" / "lr_db_common.tsv"
manifest["lr_db_common_tsv"] = str(lr_path)
manifest["common_db"] = {"source": "staged_local_lr_db_common", "n_pairs": int(len(pd.read_csv(lr_path, sep="\t")))}
manifest_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
print(f"Patched PDC manifest to use staged common LR DB: {manifest['common_db']}")
PY
fi
